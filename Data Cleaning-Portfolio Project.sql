...
DELETE
FROM duplicate_cte
WHERE row_num > 1; -- böyle bir table olmadığı için silmiyor.

-- Duplicate columnlar için (row_num = 2 olanlar için) yeni bir table yaratacağız. O table dakileri sileceğiz.
-- layoffs_staging sağ click Copy to Clipboard -> Create Statement -> buraya Paste
...
ALTER TABLE layoffs_staging2  -- date hala text formatındaydı. Bu sayede date formatına çevirdik
